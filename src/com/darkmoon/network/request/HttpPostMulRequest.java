 ...
 * @author wuwei
 */
public class HttpPostMulRequest extends Request {
	...
	public void setRequestEntity() throws IOException {
		...
		mHttpUriRequest.setHeader("Content-Type", MULTIPART_FROM_DATA
				+ ";boundary=" + BOUNDARY);
		// 首先组拼文本类型的参数
		...
		outStream.flush();
		outStream.close();
		((HttpPost) mHttpUriRequest).setEntity(new ByteArrayEntity(outStream
				.toByteArray()));
